...
private static Logger logger = LoggerFactory.getLogger(RedisUtils.class);
	...
	static {
        ...
        String host = PropertiesUtils.getInstance().getProperty("redis.host");//"192.168.0.123";
        int port = Integer.valueOf(PropertiesUtils.getInstance().getProperty("redis.port"));
        String password = null;
        Object pwd = PropertiesUtils.getInstance().getProperty("redis.password");
        if (pwd != null) {
        	password = (String)pwd;
	 ...
	 * @return
	 */
	public static String setListItemValue(String key, long index, String value) {
		String result = null;
		Jedis jedis = null;
		try {
			jedis = getResource();
			if (jedis.exists(key)) {
				...
				logger.debug("setListItemValue {} {} = {}", key, index, value);
			}
		} catch (Exception e) {
			logger.warn("setListItemValue {} {} = {}", key, index, value, e);
		} finally {
			returnResource(jedis);
		}
		return result;
	 ...
	 * @param value
	 */
	public static long deleteListItem(String key, String value) {
		long result = 0;
		Jedis jedis = null;
		try {
			jedis = getResource();
			if (jedis.exists(key)) {
				...
				logger.debug("deleteListItem {} = {}", key, value);
			}
		} catch (Exception e) {
			logger.warn("deleteListItem {} = {}", key, value, e);
		} finally {
			returnResource(jedis);
		}
		return result;
